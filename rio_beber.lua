            ...
            if Water == WaterTypes[k]["waterhash"]  then
                if IsPedOnFoot(PlayerPedId()) then
                    if IsEntityInWater(PlayerPedId()) then
                      --  DrawTxt("(SPACEBAR) wash (enter) drink", 0.50, 0.9, 0.23, 0.40, true, 223, 44, 53, 255, true)
                          --  DrawTxt("Press [~e~G~q~] to wash, [~e~ENTER~q~] to Drink Water, ", 0.50, 0.95, 0.7, 0.5, true, 255, 255, 255, 255, true)
                          ...
                          if PromptHasHoldModeCompleted(DrinkPrompt) then
                            ...
                            Citizen.Wait(17000)
                            TriggerEvent('srp:drink', 50)
                            TriggerEvent("vorpmetabolism:changeValue", "Thirst", 70) -- from client side
